...
$$('.proj-card').forEach(card => {
  card.addEventListener('mousemove', (e) => {
    ...
    card.style.transition = 'transform var(--anim-fast) var(--anim-ease)';
  });
  card.addEventListener('mouseleave', () => {
    card.style.transform = '';
    card.style.transition = 'transform var(--anim-slow) var(--anim-ease), box-shadow var(--anim) var(--anim-ease)';
  });
});
...
skillRows.forEach(row => {
  ...
  pills.forEach((pill, i) => {
    ...
    pill.style.transition = `opacity var(--anim) var(--anim-ease) ${i * 80}ms, transform var(--anim) var(--anim-ease) ${i * 80}ms`;
  });
});
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (navbar) {
    ...
    navbar.style.transition = 'opacity var(--anim) var(--anim-ease) 0.1s, transform var(--anim) var(--anim-ease) 0.1s';
